...
| **B3** | **Identidad única** | **19 sep** | `[~]` **en curso, y el salto ya funciona.** Ecosystem y Membresías, en pie: SSO por `#token=`, invitaciones, código de club, sesiones revocables (24 ago), **reconciliación aplicada** (29 ago: 0 cuentas creadas, 46 personas enlazadas) y la **herencia de plan** de §4.1 · 9 (29 ago). **Campeonatos ya entra desde el portal sin segunda contraseña** (30 ago): C1 verificador, C3 espejo, C6 frontend y el canje de C4, con el `kid` arreglado en el ecosistema. ~~**Falta**: C2 (guards), C5 (Socket.IO), C7 (roles), C8 y C9~~ **Al 26 sep 2026, hecho en código** por `PLAN-CAMPEONATOS.md`: C7 (F1/F2), C8 y C9 (F3), C4 (D8: `register` convertido), C5 de otra forma (la puerta del tatami, 26 sep) y C2 cubierto por las guardas `require_*`. **Sin desplegar.** Lo que NO está: las decisiones **8** (candado de sede) y **9** (publicar en vivo) — ver el diario de `PLAN-CAMPEONATOS.md` |
| **B3s** | Reposo y observación | 20–30 sep | `[-]` sin campeonato en octubre (D7 de `PLAN-CAMPEONATOS.md`): se observa después de desplegar el carril C |
| **🔒** | ~~**CONGELADO — campeonato del 9, 10 y 11 de octubre**~~ | ~~1–13 oct~~ | `[-]` ese campeonato no se hace (24 sep 2026) |
...
`[x]` ~~`register` condicionado a que NO haya `ECOSYSTEM_JWKS_URL`~~ — hecho el 25 sep 2026 con `ECOSYSTEM_SYNC_SECRET` como interruptor (el PC del evento lleva JWKS desde F8). Ver D8 de `PLAN-CAMPEONATOS.md`.
...
| **C2** Guards | `app/api/scoping.py` | `usuario_actual()` lee claims y resuelve el espejo; `@requiere_scope` / `@requiere_rol` sustituyen a `@jwt_required()` | `[x]` **cubierto de otra forma**: el scope se mira UNA vez, al canjear el pase (`/auth/sesion`), y después mandan `require_admin` / `require_maestro` / `require_personal` (F3). Decoradores nuevos no hacen falta |
| **C3** Espejo | `models/usuario.py`, `schema_compat.py`, `app/espejo.py` | `eco_sub` + alta/enlace automático. `schema_compat.py` ya es el mecanismo para añadir columnas sin migraciones | `[x]` **HECHO** (30 ago). Tres caminos: ya tiene espejo → se usa; existe por correo → se ENLAZA; no existe → se crea, **y solo si el pase trae rol que opere**. Un alumno no crea fila aquí: una federación de doscientos alumnos no son doscientas filas en la consola. **El rol local manda** sobre el del pase (como Academy): el pase solo decide el rol al crear. El maestro **estrena su club** preguntándoselo al ecosistema con su propio pase, solo si no tiene y solo si es maestro. Y `eco_sub` es `uuid` en PostgreSQL —la creó la reconciliación—: comparar ese UUID con la cadena del pase daba distinto siempre, y en SQLite no se veía |
| **C4** Retirar la ~~emisión~~ **creación de cuentas** | `app/api/auth.py` | ~~Fuera `login`~~ (**ver decisión 12**: el login se queda, es la marcha atrás). Fuera **`register`** y las contraseñas nuevas. Se conservan `/me`, `/logout`, `/socket-ticket`, `/clubes`. `POST /auth/sesion` pasa a ser el canje SSO | `[x]` **el canje, hecho** (30 ago), y **`register` convertido el 25 sep 2026** (D8 de `PLAN-CAMPEONATOS.md`: en internet el juez nace en DINAMYT; en el PC del evento, como siempre): `/auth/sesion` acepta las dos puertas —el pase del ecosistema y el token propio del QR— y abre la cookie de aquí, con **12 h** en vez de 72 cuando viene del pase (allá la sesión se puede revocar; esta cookie ya no depende de él). **Retirar el login propio NO se hace antes del campeonato**: es la marcha atrás del 9 de octubre y lo que sostiene el modo local |
| **C5** Socket.IO | `sockets/combate_ns.py:477` | `decode_token` → el verificador de C1. El token sigue viajando en el `auth` del socket | `[x]` **26 sep 2026, de otra forma**: el socket no ve pases RS256 (usa el ticket de la sesión de aquí), pero ahora EXIGE identidad para puntuar —superadmin, dueño o juez asignado— y ata el juez de cada punto a la conexión. Antes era opcional y cualquiera tomaba el tatami |
...
| **C7** Roles | varios | `admin→admin`, `juez→judge`, `maestro→coach`. `es_superadmin` se lee del token | `[x]` F1 y F2 de `PLAN-CAMPEONATOS.md` (roles múltiples, el portal da y quita lo suyo: D9). `es_superadmin` sigue sin viajar, a propósito |
| **C8** Competidor ↔ persona | `models/competidor.py`, `schema_compat.py` | Columna `eco_sub` en `competidores` (nullable). **Hoy no existe ningún enlace entre un competidor y una persona**: `created_by` dice quién lo *inscribió*, no quién *es*. Sin esta columna, al terminar B3 el alumno entra con su cuenta pero el sistema sigue sin saber cuáles de esos competidores es él | `[x]` **13 sep 2026**, F3 de `PLAN-CAMPEONATOS.md`. Texto también en PostgreSQL, sin `unique` (una ficha por workspace puede ser de la misma persona), y viaja en el paquete |
| **C9** Reclamar lo competido | ecosystem + Campeonatos | Al crear cuenta o al entrar por primera vez, buscar competidores con el mismo `documento` y sin dueño, y proponerlos: «encontramos 3 participaciones a tu nombre, ¿son tuyas?». Funciona **hacia atrás**, con lo competido hace años | `[x]` **13 sep 2026**, F3 — distinto de lo escrito: no se busca solo por documento (bastaría con conocer la cédula de alguien). La persona la reclama desde `/mi-panel` con documento **y** fecha de nacimiento, o el administrador la enlaza a mano |
...
| **B3** | Identidad única (§4): ecosystem-api → Membresías (poco) → Campeonatos (mucho) → reconciliación (§2.4) → aviso a la gente | **19 sep** | **Alto** | `[~]` ecosystem-api y Membresías hechos, reconciliación aplicada (29 ago). De Campeonatos: **entrar desde el portal ya funciona** (30 ago); los roles múltiples, las guardas del personal y «mis campeonatos» (el panel del competidor) están desde el 13 sep (F1–F3 de `PLAN-CAMPEONATOS.md`). Queda la organización (F4) |
